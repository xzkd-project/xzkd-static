...
async def fetch_course_info(session: aiohttp.ClientSession, semester_path: str, _courses: [Course], sem, progress_bar):
    async with sem:
...
async def fetch_semester(session: aiohttp.ClientSession, curriculum_path: str, semester_id: str):
    ...
    tasks = [fetch_course_info(session, semester_path, _courses, sem, progress_bar) for _courses in course_chunks]
...
async def make_curriculum():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for semester in tqdm(semesters):
            await fetch_semester(session, curriculum_path, str(semester.id))
